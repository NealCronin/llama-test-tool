...
class ContextKvPresetDialog(_PresetDialog):
    def __init__(self, catalog, parent=None) -> None:
        ...
        self.context = QLineEdit("131072")
        self.k_type = QComboBox(); self.k_type.addItems(values)
        self.v_type = QComboBox(); self.v_type.addItems(values)
...
class DeviceSplitPresetDialog(_PresetDialog):
    def __init__(self, parent=None) -> None:
        ...
        form = QFormLayout(self)
        self.devices = QLineEdit("Vulkan0,CUDA0")
        self.mode = QComboBox(); self.mode.addItems(("none", "layer", "row", "tensor")); self.mode.setCurrentText("layer")
        self.tensor_split = QLineEdit("2,1")
        self.main_gpu = QLineEdit("0")
        self.gpu_layers = QLineEdit("all")
        self.all_moe_cpu = QCheckBox("Keep all MoE on CPU (-cmoe)")
        self.cpu_moe_layers = QLineEdit()
...
class CustomMtpPresetDialog(_PresetDialog):
    ...
    def values(self) -> dict[str, list[str]]:
        ...
        result = {"--spec-draft-model": [path], "--spec-type": ["draft-mtp"], "--cache-type-k-draft": [self.k_type.currentText()], "--cache-type-v-draft": [self.v_type.currentText()]}
